...
1. 课件生成：根据章节主题、班级薄弱点、教学目标生成 LaTeX Beamer 源文件、PDF 预览和最终 PPTX，并自动匹配章节图片资产。
...
- `outputs/基因组数据中的多重检验校正_课件.tex`
- `outputs/基因组数据中的多重检验校正_课件.pdf`
- `outputs/基因组数据中的多重检验校正_课件.pptx`
...
- Python/R 运行时会注入题目所需的少量示例数据；运行超时时间为 30 秒。
